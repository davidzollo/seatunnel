...
public class KingbaseSchema {
    ...
    public KingbaseSchema(
            final KingbaseConnectorConfig connectorConfig, Map<TableId, CatalogTable> tableMap) {
    ...
    private TableChanges.TableChange readTableSchema(JdbcConnection jdbc, TableId tableId) {
        // Because the catalog is null in the kingbaseConnection.readSchema method
        TableId tableIdWithoutCatalog = new TableId(null, tableId.schema(), tableId.table());

        KingbaseConnection kingbaseConnection = (KingbaseConnection) jdbc;
        Tables tables = new Tables();
        try {
            kingbaseConnection.readSchema(
                    tables,
