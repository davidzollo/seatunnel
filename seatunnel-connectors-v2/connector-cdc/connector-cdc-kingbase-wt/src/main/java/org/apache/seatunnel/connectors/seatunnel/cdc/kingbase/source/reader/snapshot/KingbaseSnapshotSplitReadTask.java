...
@Slf4j
public class KingbaseSnapshotSplitReadTask extends AbstractSnapshotChangeEventSource {
    ...
    private void createDataEvents(SqlSeverSnapshotContext snapshotContext, TableId tableId)
        ...
        // Kingbase schema identifiers do not include the database name.
    ...
    private Object readField(ResultSet rs, int columnIndex, Column column) throws SQLException {
        final ResultSetMetaData metaData = rs.getMetaData();
        final int columnType = metaData.getColumnType(columnIndex);

        if (column.nativeType() == KingbaseOid.BIT || column.nativeType() == KingbaseOid.VARBIT) {
            // Kingbase JDBC treats BIT(n) as boolean in getObject(), which fails for values like
            // B'1010'. Reading the string representation keeps snapshot and incremental types
            // aligned.
