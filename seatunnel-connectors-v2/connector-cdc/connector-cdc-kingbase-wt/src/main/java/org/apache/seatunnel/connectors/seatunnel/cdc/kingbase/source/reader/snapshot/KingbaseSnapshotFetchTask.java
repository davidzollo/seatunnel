...
@Slf4j
public class KingbaseSnapshotFetchTask implements FetchTask<SourceSplitBase> {
    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseSnapshotFetchTask.class);

    private final SnapshotSplit split;

    private volatile boolean taskRunning = false;

    private KingbaseSnapshotSplitReadTask snapshotSplitReadTask;

    public KingbaseSnapshotFetchTask(SnapshotSplit split) {
        this.split = split;
    }
    ...
    public void execute(Context context) throws Exception {
        ...
        snapshotSplitReadTask =
                new KingbaseSnapshotSplitReadTask(
        ...
        if (!context.isExactlyOnce()) {
            taskRunning = false;
            if (changed) {
                LOGGER.debug("Skip merge changelog(exactly-once) for snapshot split {}", split);
            }
            return;
        }
        ...
        if (true) {
            dispatchBinlogEndEvent(
                    ...
                    ((KingbaseSourceFetchTaskContext) context).getDispatcher());
            taskRunning = false;
            return;
        }
    }
