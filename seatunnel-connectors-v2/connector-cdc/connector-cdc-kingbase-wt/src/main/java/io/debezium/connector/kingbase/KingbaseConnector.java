...
/**
 * A Kafka Connect source connector that creates tasks which use Kingbase streaming replication off
 ...
 * KingbaseConnectorConfig}.
 *
 * @author Horia Chiorean
 */
public class KingbaseConnector extends RelationalBaseSourceConnector {
    ...
    public ConfigDef config() {
        return KingbaseConnectorConfig.configDef();
    }
    ...
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        ...
        final ConfigValue slotNameValue =
                configValues.get(KingbaseConnectorConfig.SLOT_NAME.name());
        final ConfigValue pluginNameValue =
                configValues.get(KingbaseConnectorConfig.PLUGIN_NAME.name());
        ...
        final KingbaseConnectorConfig postgresConfig = new KingbaseConnectorConfig(config);
        ...
        try (KingbaseConnection connection =
                new KingbaseConnection(postgresConfig.getJdbcConfig())) {
            try {
                ...
                if (!"logical".equals(walLevel)) {
                    final String errorMessage =
                            "Kingbase server wal_level property must be \"logical\" but is: "
                                    + walLevel;
                ...
                if (!connection.queryAndMap(
                    ...
                    final String errorMessage =
                            "Kingbase roles LOGIN and REPLICATION are not assigned to user: "
                                    + connection.username();
                    LOGGER.error(errorMessage);
                    hostnameValue.addErrorMessage(errorMessage);
                }
    ...
    protected Map<String, ConfigValue> validateAllFields(Configuration config) {
        return config.validate(KingbaseConnectorConfig.ALL_FIELDS);
    }
}
