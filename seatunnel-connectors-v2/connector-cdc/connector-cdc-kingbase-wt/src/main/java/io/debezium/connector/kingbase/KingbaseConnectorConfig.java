...
/**
 * The configuration properties for the {@link KingbaseConnector}
 *
 * @author Horia Chiorean
 */
public class KingbaseConnectorConfig extends RelationalDatabaseConnectorConfig {

    private static final Logger LOGGER = LoggerFactory.getLogger(KingbaseConnectorConfig.class);
    private static final String DEFAULT_UNAVAILABLE_VALUE_PLACEHOLDER =
            "__debezium_unavailable_value";
    private static final String KINGBASE_PGOUTPUT_PLUGIN_NAME = "syslogical_output";
    ...
    public enum LogicalDecoder implements EnumeratedValue {
        PGOUTPUT("pgoutput") {
            ...
            public String getPostgresPluginName() {
                // Kingbase exposes the pgoutput-compatible logical stream through
                // syslogical_output.so instead of PostgreSQL's pgoutput.so.
                return KINGBASE_PGOUTPUT_PLUGIN_NAME;
            }
    ...
    public static final Field PLUGIN_NAME =
            Field.create("plugin.name")
                    ...
                    .withDescription(
                            "The logical decoding protocol used by Kingbase CDC. "
                                    + "For pgoutput, Kingbase uses the built-in server plugin '"
                                    + KINGBASE_PGOUTPUT_PLUGIN_NAME
                                    + "'. "
    ...
    public static final Field SLOT_NAME =
            Field.create("slot.name")
                    ...
                    .withValidation(KingbaseConnectorConfig::validateReplicationSlotName)
                    .withDescription(
    ...
    public static final Field SSL_SOCKET_FACTORY =
            Field.create(DATABASE_CONFIG_PREFIX + "sslfactory")
                    ...
                    .withImportance(Importance.MEDIUM)
                    .withDescription(
                            "A name of class to that creates SSL Sockets. Use org.kingbase.ssl.NonValidatingFactory to disable SSL validation in development environments");
    ...
    public static final Field LOGICAL_DECODING_MESSAGE_PREFIX_EXCLUDE_LIST =
            Field.create("message.prefix.exclude.list")
                    ...
                    .withValidation(
                            Field::isListOfRegex,
                            KingbaseConnectorConfig::validateLogicalDecodingMessageExcludeList)
                    .withDescription(
    ...
    public static final Field TRUNCATE_HANDLING_MODE =
            Field.create("truncate.handling.mode")
                    ...
                    .withWidth(Width.MEDIUM)
                    .withImportance(Importance.MEDIUM)
                    .withValidation(KingbaseConnectorConfig::validateTruncateHandlingMode)
                    .withDescription(
    ...
    public static final Field TOASTED_VALUE_PLACEHOLDER =
            Field.create("toasted.value.placeholder")
                    ...
                    .withDefault(DEFAULT_UNAVAILABLE_VALUE_PLACEHOLDER)
                    .withImportance(Importance.MEDIUM)
                    .withValidation(KingbaseConnectorConfig::validateToastedValuePlaceholder)
                    .withDescription(
                            "Specify the constant that will be provided by Debezium to indicate that "
    ...
    public KingbaseConnectorConfig(Configuration config) {
        super(
                config,
        ...
        this.truncateHandlingMode =
                TruncateHandlingMode.parse(
                        config.getString(KingbaseConnectorConfig.TRUNCATE_HANDLING_MODE));
        ...
        String hstoreHandlingModeStr =
                config.getString(KingbaseConnectorConfig.HSTORE_HANDLING_MODE);
        ...
        this.intervalHandlingMode =
                IntervalHandlingMode.parse(
                        config.getString(KingbaseConnectorConfig.INTERVAL_HANDLING_MODE));
    ...
    protected Duration statusUpdateInterval() {
        return Duration.ofMillis(
                getConfig().getLong(KingbaseConnectorConfig.STATUS_UPDATE_INTERVAL_MS));
    }
    ...
    protected Duration xminFetchInterval() {
        return Duration.ofMillis(getConfig().getLong(KingbaseConnectorConfig.XMIN_FETCH_INTERVAL));
    }
    ...
    protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(
        ...
        switch (version) {
            case V1:
                return new LegacyV1KingbaseSourceInfoStructMaker(
                        Module.name(), Module.version(), this);
            default:
                return new KingbaseSourceInfoStructMaker(Module.name(), Module.version(), this);
        }
    }
    ...
    public Connection getConnection(KingbaseConnectorConfig config) {
        // Replication must follow the exact JDBC URL chosen by the datasource so V8/V9 bridge
        // ports and sslmode query parameters remain consistent with the catalog connection.
        String sourceURL = config.getJdbcConfig().getString("url");
        if (Strings.isNullOrEmpty(sourceURL)) {
            sourceURL =
                    "jdbc:kingbase8://" + hostname() + ":" + port() + "/" + config.databaseName();
        }
        Connection connection = null;
        try {
            connection = DriverManager.getConnection(sourceURL, replicationConnectionProperties());
            ...
            LOGGER.error("Create Kingbase connection failed.", exp);
            throw new SeaTunnelException("Create Kingbase connection failed.", exp);
        }
        return connection;
    }

    @VisibleForTesting
    Properties replicationConnectionProperties() {
        Properties properties = new Properties();
        properties.putAll(getJdbcConfig().asProperties());
        KBProperty.USER.set(properties, user());
        KBProperty.PASSWORD.set(properties, password());
        // Logical replication requires these connection properties; without them Kingbase opens a
        // normal SQL connection and the replication API cannot start streaming.
        KBProperty.ASSUME_MIN_SERVER_VERSION.set(properties, "9.4");
        KBProperty.REPLICATION.set(properties, "database");
        KBProperty.PREFER_QUERY_MODE.set(properties, "simple");
        return properties;
    }
