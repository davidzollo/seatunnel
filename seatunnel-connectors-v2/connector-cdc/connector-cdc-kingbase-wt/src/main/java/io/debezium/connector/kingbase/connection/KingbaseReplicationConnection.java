...
/**
 * Implementation of a {@link ReplicationConnection} for Kingbase. Note that replication connections
...
public class KingbaseReplicationConnection extends JdbcConnection implements ReplicationConnection {
    ...
    private final KingbaseConnectorConfig.AutoCreateMode publicationAutocreateMode;
    private final KingbaseConnectorConfig.LogicalDecoder plugin;
    private final boolean dropSlotOnClose;
    private final KingbaseConnectorConfig originalConfig;
     ...
     * @param typeRegistry registry with Kingbase types
    ...
    private KingbaseReplicationConnection(
            ...
            KingbaseConnectorConfig.AutoCreateMode publicationAutocreateMode,
            KingbaseConnectorConfig.LogicalDecoder plugin,
        ...
        super(
                ...
                KingbaseConnection.FACTORY,
                KingbaseReplicationConnection.class::getClassLoader,
                "",
                "");
        ...
        // Publication management and IDENTIFY_SYSTEM run through a dedicated replication JDBC
        // connection, so it must see the same JDBC transport flags as the ordinary metadata
        // connection (for example sslmode=disable against plain local Docker deployments).
    ...
    private ServerInfo.ReplicationSlot getSlotInfo() throws SQLException, InterruptedException {
        try (KingbaseConnection connection =
                new KingbaseConnection(originalConfig.getJdbcConfig())) {
    ...
    protected void initPublication() {
        ...
        if (KingbaseConnectorConfig.LogicalDecoder.PGOUTPUT.equals(plugin)) {
    ...
    // Temporary replication slots is a new feature of Kingbase 10
    ...
    public ReplicationStream startStreaming(Lsn offset, WalPositionLocator walPosition)
        ...
        int maxRetries = config().getInteger(KingbaseConnectorConfig.MAX_RETRIES);
        int delay = config().getInteger(KingbaseConnectorConfig.RETRY_DELAY_MS);
        int tryCount = 0;
        while (true) {
            try {
    ...
    public Optional<SlotCreationResult> createReplicationSlot() throws SQLException {
        ...
        // For pgoutput specifically, the publication must be created prior to the slot. The
        // decoderbufs first path does not use publication_names or other pgoutput slot options.
        initPublication();
    ...
    private ReplicationStream createReplicationStream(
        ...
        try {
            ...
            if (e.getMessage().matches("(?s)ERROR: option .* is unknown.*")) {
                ...
                throw new ConnectException(
                        "The offset to start reading from has been removed from the database write-ahead log. Create a new snapshot and consider setting of Kingbase parameter wal_keep_segments = 0.");
            } else {
                throw e;
            }
        }
    ...
    private KBReplicationStream startPgReplicationStream(
            final Lsn lsn,
            BiFunction<
    ...
    protected static void defaultSettings(Configuration.Builder builder) {
        ...
        KingbaseConnection.defaultSettings(builder);
    ...
    protected static class ReplicationConnectionBuilder implements Builder {
        ...
        private KingbaseConnectorConfig.AutoCreateMode publicationAutocreateMode =
                KingbaseConnectorConfig.AutoCreateMode.FILTERED;
        private KingbaseConnectorConfig.LogicalDecoder plugin =
                KingbaseConnectorConfig.LogicalDecoder.PGOUTPUT;
        ...
        public ReplicationConnectionBuilder withPlugin(
                final KingbaseConnectorConfig.LogicalDecoder plugin) {
            assert plugin != null;
            this.plugin = plugin;
            return this;
        }
        ...
        public ReplicationConnection build() {
            ...
            return new KingbaseReplicationConnection(
                    config,
                    slotName,
                    publicationName,
                    tableFilter,
