...
import io.debezium.connector.kingbase.connection.ReplicationStream;
import io.debezium.connector.kingbase.connection.WalPositionLocator;
...
public class KingbaseStreamingChangeEventSource
        implements StreamingChangeEventSource<KingbaseOffsetContext> {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(KingbaseStreamingChangeEventSource.class);
    ...
    public KingbaseStreamingChangeEventSource(
            KingbaseConnectorConfig connectorConfig,
            Snapshotter snapshotter,
            KingbaseConnection connection,
            KingbaseEventDispatcher<TableId> dispatcher,
    ...
    @Override
    public void execute(ChangeEventSourceContext context, KingbaseOffsetContext offsetContext)
        ...
        if (!hasStartLsnStoredInContext) {
            offsetContext =
                    KingbaseOffsetContext.initialContext(connectorConfig, connection, clock);
        }
                                    ...
                                    boolean dispatched =
                                            message.getOperation() != Operation.NOOP
                                                    && dispatcher.dispatchDataChangeEvent(
                                                            tableId,
                                                            new KingbaseChangeRecordEmitter(
                                                                    offsetContext,
                                                                    clock,
                                                                    connectorConfig,
                                                                    schema,
    ...
    private void commitMessage(KingbaseOffsetContext offsetContext, final Lsn lsn)
            throws SQLException, InterruptedException {
        lastCompletelyProcessedLsn = lsn;
    ...
    public void commitOffset(Map<String, ?> offset) {
        try {
            ...
            final Lsn commitLsn =
                    Lsn.valueOf((Long) offset.get(KingbaseOffsetContext.LAST_COMMIT_LSN_KEY));
            final Lsn changeLsn =
                    Lsn.valueOf(
                            (Long)
                                    offset.get(
    ...
    @FunctionalInterface
    public static interface KingbaseConnectionSupplier {
