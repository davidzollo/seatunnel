 ...
 */
@ThreadSafe
public class KingbaseTaskContext extends CdcSourceTaskContext {

    protected static final Logger LOGGER = LoggerFactory.getLogger(KingbaseTaskContext.class);
    ...
    protected KingbaseSchema schema() {
        return schema;
    }

    protected KingbaseConnectorConfig config() {
        return config;
    }

    public void refreshSchema(KingbaseConnection connection, boolean printReplicaIdentityInfo)
            throws SQLException {
        schema.refresh(connection, printReplicaIdentityInfo);
    }

    Long getSlotXmin(KingbaseConnection connection) throws SQLException {
    ...
    private SlotState getCurrentSlotState(KingbaseConnection connection) throws SQLException {
    ...
    public ReplicationConnection createReplicationConnection(boolean doSnapshot)
        ...
        if (dropSlotOnStop) {
            LOGGER.warn(
                    ...
                    KingbaseConnectorConfig.DROP_SLOT_ON_STOP.name());
        }
    ...
    KingbaseConnectorConfig getConfig() {
        return config;
    }
}
