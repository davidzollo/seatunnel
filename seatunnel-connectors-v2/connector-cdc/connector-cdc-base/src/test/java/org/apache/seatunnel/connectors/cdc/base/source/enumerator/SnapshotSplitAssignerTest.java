...
/** Tests restore-time add-back handling for snapshot splits in {@link SnapshotSplitAssigner}. */
public class SnapshotSplitAssignerTest {

    /** Verifies that a checkpointed finished split is not replayed after restore. */
    @Test
    ...
    /**
     * Verifies that a finished split without completed watermark is still replayed after restore.
     */
    @Test
    ...
    /** Creates a restored assigner state that matches the failover recovery path under test. */
    private SnapshotSplitAssigner<?> createRestoredSnapshotSplitAssigner(
        ...
        return new SnapshotSplitAssigner<>(context, 10, checkpointState, null, null);
    }

    /** Creates a finished snapshot split with low/high watermarks for restore assertions. */
    ...
    /** Minimal offset implementation used by the restore regression fixtures. */
