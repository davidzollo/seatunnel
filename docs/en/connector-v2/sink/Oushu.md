...
|                                         Oushu Data Type                                          |                                                              SeaTunnel Data Type                                                               |
|--------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------|
| BOOL<br/>                                                                                        | BOOLEAN                                                                                                                                        |
| BYTEA<br/>                                                                                       | BYTES                                                                                                                                          |
...
| compatible_mode                           | String  | Yes      | oushu                        |
...
The write modes support five modes: SQL, COPY, COPY_SQL.
- SQL (default): The traditional SQL mode using JDBC, supporting both full and incremental writes.
- COPY: Import data using the COPY command (requires client support such as Postgres, OddDB), only supports full writes.
- COPY_SQL: Import data using the COPY command (requires client support), and dynamically switches to SQL mode for writing if there is incremental data.

### temp_table_name[String]

Temporary table name, used only when write_mode is MERGE and COPY_MERGE. The default is empty, which means using the parameter `target_table_name + "_tmp"` as the temporary table name.

### compatible_mode[String]

If using the Oushu target database, the compatible_mode parameter must be specified as `oushu`.

### Tips

> If partition_column is not set, it will run in single concurrency, and if partition_column is set, it will be executed  in parallel according to the concurrency of tasks.
>
> If using COPY write mode, it does not support target tables that contain bytea type columns, and unsupported Array/Map/Raw types for copy write mode.
...
### User COPY write mode:

> This example use copy write mode to import data from source
...
### User COPY_SQL write mode for upsert :

> This example use copy_sql write mode to import data from source and switch to SQL mode for writing if there is incremental data.
