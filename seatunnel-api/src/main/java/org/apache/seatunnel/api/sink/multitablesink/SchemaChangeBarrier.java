...
final class SchemaChangeBarrier {
    ...
    /**
     * Executes the actual shared-sink schema mutation after every queue has drained older rows and
     * reached the barrier.
     */
    @FunctionalInterface
    interface Dispatcher {

        /** Applies the schema change to every sub-writer selected for this barrier. */
        void dispatch(SchemaChangeEvent event) throws IOException;
    }

    /** Carries the schema change that all queue workers are coordinating around. */
    private final SchemaChangeEvent event;
    /** Records how many queue workers must arrive before the mutation can run. */
    private final int participantCount;
    /** Performs the actual shared-sink schema mutation once all queues are ready. */
    private final Dispatcher schemaChangeDispatcher;
    /** Counts how many queue workers have already reached the barrier. */
    private final AtomicInteger arrivedQueues = new AtomicInteger(0);
    /** Prevents success and failure completion paths from releasing the barrier twice. */
    private final AtomicBoolean completedOrFailed = new AtomicBoolean(false);
    /** Releases every parked queue worker after the schema change either finishes or fails. */
    private final CountDownLatch completed = new CountDownLatch(1);
    /** Stores the first failure that should be surfaced back to the coordinator thread. */
    private final AtomicReference<Throwable> failure = new AtomicReference<>();

    /**
     * Creates one shared barrier for a single schema-change event that must be coordinated across
     * every queue worker.
     */
    SchemaChangeBarrier(
     ...
     * mutation while the others stay parked behind the same completion latch.
     */
    ...
    /** Re-throws the first recorded failure through the barrier's public IOException contract. */
