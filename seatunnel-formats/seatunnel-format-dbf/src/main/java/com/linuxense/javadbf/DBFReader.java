...
public class DBFReader extends DBFBase implements Closeable {
     ...
     *
     * <p>When this constructor returns the object will have completed reading the header (meta
     * date) and header information can be queried there on. And it will be ready to return the
     * first row.
     *
     * @param in the InputStream where the data is read from.
     ...
     *     autedetected from dbf file
     * @param showDeletedRows can be used to identify records that have been deleted.
     * @param supportExtendedCharacterFields Defines whether 2-byte (extended) length character
     *     fields should be supported (see DBFField.adjustLengthForLongCharSupport(), default:
     *     true).
     */
    public DBFReader(
            InputStream in,
    ...
    /**
     * Set the list of memo fields to be readed as binary. By default memo fields are retrieved as
     * Strings, but if you know they store binary data you can set the names of the memo fields to
     * be recovered unproceesded as byte[]
     *
            ...
            do {
                try {
                    ...
                    if (t_byte == END_OF_DATA
                            && this.header.numberOfRecords == this.internalRecord) {
                        return null;
                    }
                    isDeleted = t_byte == '*';
                    ...
                    if (field.isSystem() || field.getType() == DBFDataType.NULL_FLAGS) {
                        if (field.getType() == DBFDataType.NULL_FLAGS && o instanceof BitSet) {
                            ...
                            for (int j = 0; j < this.header.fieldArray.length; j++) {
                                ...
                                if (field1.getType() == DBFDataType.VARBINARY
                                        || field1.getType() == DBFDataType.VARCHAR) {
                                    currentIndex++;
                ...
                try {
                    GregorianCalendar calendar =
                            new GregorianCalendar(
                                    Integer.parseInt(
                                            new String(t_byte_year, DBFStandardCharsets.US_ASCII)),
                                    Integer.parseInt(
                                                    new String(
                                                            t_byte_month,
                                                            DBFStandardCharsets.US_ASCII))
                                            - 1,
                                    Integer.parseInt(
                                            new String(t_byte_day, DBFStandardCharsets.US_ASCII)));
            ...
            case TIMESTAMP_DBASE7:
                ...
                if (days == 0 && time == 0) {
                    ...
                    calendar.setTimeInMillis(
                            days * MILLISECS_PER_DAY + TIME_MILLIS_1_1_4713_BC + time);
                    calendar.add(
    ...
    private Object readMemoField(DBFField field) throws IOException {
        ...
        if (this.memoFile != null && nBlock != null && nBlock.intValue() > 0) {
            ...
            if (type == DBFDataType.MEMO
                    && fieldsAsBinary.contains(field.getName().toUpperCase())) {
    ...
    /**
     * Safely skip bytesToSkip bytes (in some bufferd scenarios skip doesn't really skip all
     * requested bytes)
     *
    ...
    /**
     * Sets the memo file (DBT or FPT) where memo fields will be readed. If no file is provided,
     * then this fields will be null.
     *
