...
public class DBFWriter extends DBFBase implements java.io.Closeable {
    ...
    /**
     * Creates a DBFWriter wich write data to the given OutputStream. Uses default charset
     * iso-8859-1
     *
     ...
     * @exception DBFException if the passed in file does exist but not a valid DBF file, or if an
     *     IO error occurs.
     */
            ...
            if (dbfFile.length() == 0) {
                if (charset != null) {
                    if (DBFCharsetHelper.getDBFCodeForCharset(charset) == 0
                            && !DBFStandardCharsets.UTF_8.equals(charset)) {
    ...
    /**
     * Sets fields definition. To keep maximum compatibility a maximum of 255 columns should be
     * used.
    ...
    public void setFields(DBFField[] fields) {
        ...
        for (DBFField field : fields) {
            ...
            //				throw new DBFException(
            //						"Field " + field.getName() + " is of type " + field.getType() + " that is only
            // supported for writting in ADVANCED FILE FORMAT");
            //			}
        ...
        for (int j = 0; j < this.header.fieldArray.length; j++) {
            ...
            switch (this.header.fieldArray[j].getType()) {
                ...
                case AUTOINCREMENT:
                    if (objectArray[j] != null) {
                        dataOutput.writeInt(
                                DBFUtils.littleEndian(((Number) objectArray[j]).intValue()));
                    } else {
                ...
                case TIMESTAMP_DBASE7:
                    ...
                    // 8 bytes - two longs,
                    // first for date, second for time.  The date is the number of days since
                    // 01/01/4713 BC.
                    // Time is hours * 3600000L + minutes * 60000L + Seconds * 1000L

                    int days =
                            (int)
                                    (((c.getTimeInMillis() - millis) / MILLISECS_PER_DAY)
                                            - (TIME_MILLIS_1_1_4713_BC / MILLISECS_PER_DAY));
                ...
                default:
                    throw new DBFException(
                            "Unknown field type " + this.header.fieldArray[j].getType());
            }
        }
    }
