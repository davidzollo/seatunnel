...
public class DBFField {
     ...
     *
     * <p>The data in the DataInputStream object is supposed to be organised correctly and the
     * stream "pointer" is supposed to be positioned properly.
     *
     * @param in DataInputStream
     * @param charset charset to use
     * @param useFieldFlags If the file can store field flags (setting this to false ignore any data
     *     in byes 18-19)
     * @param supportExtendedCharacterFields True if 2-byte (extended) length character fields
     *     should be supported, see adjustLengthForLongCharSupport()
    ...
    protected static DBFField createFieldDB7(
            DataInput in, Charset charset, boolean supportExtendedCharacterFields)
            throws IOException {
    ...
    private static void adjustLengthForLongCharSupport(DBFField field) {
        // if field type is char or varchar, then read length and decimalCount as one number to
        // allow char fields to be
        ...
        if (field.type == DBFDataType.CHARACTER || field.type == DBFDataType.VARCHAR) {
            // Ensure length and decimalCount are non-negative
            int length = field.length & 0xFF; // Ensure length is treated as unsigned byte
            int decimalCount =
                    field.decimalCount & 0xFF; // Ensure decimalCount is treated as unsigned byte
            field.length =
                    (length | (decimalCount << 8))
                            & 0xFFFF; // Combine and ensure result is within 0-65535
     ...
     *
     * <p>If the field is specified to hold integral values the value returned by this method will
     * be zero.
     *
